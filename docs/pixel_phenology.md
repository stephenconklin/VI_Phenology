...
| `{VI}_{region_label}_pixel_metrics_overview.png` | `{PIXEL_OUTPUT_DIR}/{region_label}/` |
| `{VI}_{region_label}_pixel_metrics_overview.html` | `{PIXEL_OUTPUT_DIR}/{region_label}/` |
| `pixel_phenology_{YYYYMMDD_HHMMSS}.log` | `{PIXEL_OUTPUT_DIR}/` |

The overview PNG and HTML are generated by default. Disable individually via `PIXEL_OVERVIEW_FIGURE=false` / `PIXEL_OVERVIEW_HTML=false` in `run_phenology.sh`, or `--no-overview-figure` / `--no-overview-html` on the CLI.
...
| `--no-overview-figure` | `PIXEL_OVERVIEW_FIGURE=false` | *(overview generated)* | Skip the print-quality 19-panel overview PNG |
| `--no-overview-html` | `PIXEL_OVERVIEW_HTML=false` | *(overview generated)* | Skip the interactive Plotly HTML overview |
