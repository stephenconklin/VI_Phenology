...
All output types are enabled by default. Disable any combination in `config.local.env`:
...
└── Mesa_Verde/                                       ← one subfolder per region
    ...
    ├── NDVI_Mesa_Verde_pixel_metrics_overview.png    ← generated by default
    └── NDVI_Mesa_Verde_pixel_metrics_overview.html   ← generated by default
```
...
| `{VI}_{region_label}_pixel_metrics_overview.png` | Print-quality 4×5 panel overview of all 19 metrics (generated by default) |
| `{VI}_{region_label}_pixel_metrics_overview.html` | Interactive Plotly HTML overview with hover-enabled pixel coordinates and values (generated by default) |
| `pixel_phenology_{timestamp}.log` | Timestamped log file written to `--output-dir` root |

Disable overview outputs via `PIXEL_OVERVIEW_FIGURE=false` / `PIXEL_OVERVIEW_HTML=false` in
`config.local.env`, or `--no-overview-figure` / `--no-overview-html` on the CLI.
...
| File | Description |
|------|-------------|
| `{VI}_{region_label}_per_year.tif` | N_years × 3 bands: `year{YYYY}_median`, `year{YYYY}_p05`, `year{YYYY}_p95` per calendar year |
| `{VI}_{region_label}_per_month.tif` | 36 bands: `month{MM}_median/p05/p95` for each calendar month (per-year-then-average method) |
| `{VI}_{region_label}_per_doy.tif` | 1095 bands: `doy{DDD}_median/p05/p95` for each day-of-year (raw obs pooled across all years) |
| `datacube_to_geotiff_{timestamp}.log` | Timestamped log file written to `--output-dir` root |

All GeoTiffs: LZW-compressed, 256×256 tiled, float32, NoData = `9.96920996838687e+36`
(CF float32 fill value). Band descriptions accessible via `gdalinfo -mdd all` or
`rasterio.open().descriptions`.

Suppress individual products with `GEOTIFF_PER_YEAR=false`, `GEOTIFF_PER_MONTH=false`,
`GEOTIFF_PER_DOY=false` in `config.local.env` (or `--skip-per-year`, `--skip-per-month`,
`--skip-per-doy` on the CLI). For large regions, `--skip-per-doy` is recommended — the
1095-band product can exceed 4 GB.

For full details on the datacube_to_geotiff pipeline, see
[datacube_to_geotiff Pipeline](datacube_to_geotiff.md).
